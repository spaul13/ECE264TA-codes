...
#include <stdio.h>
#include <stdlib.h>
TreeNode * TreeNode_create(unsigned char letter)
{
...
void Tree_createCode(TreeNode * tn, unsigned char * buff, int level,
		     int * * codebook)
{
  if (tn == NULL)
    {
  ...
  if (((tn -> left) == NULL) &&
      ((tn -> right) == NULL))
    {
      unsigned char row = tn -> letter;
      for (int ind = 0; ind < level; ind ++)
	{
	  codebook[row][ind] = buff[ind];
	}
      return;
    }
  buff[level] = 0;
  Tree_createCode(tn -> left, buff, level + 1, codebook);

  buff[level] = 1;
  Tree_createCode(tn -> right, buff, level + 1, codebook);
}
